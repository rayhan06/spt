...
CKEDITOR.dialog.add( 'ImageMaps', function( editor )
{
	...
	var lang = editor.lang.imagemaps,
		...
		spacerImg = CKEDITOR.getUrl( 'images/spacer.gif' );
...
//		CKEDITOR.scriptLoader.load( plugin.path + "dialog/imgmap_packed.js", show);
	...
	var content = "";
	content += '.imgmapButton {cursor:pointer; background: url("' + plugin.path + 'images/sprite.png") no-repeat top left; width: 16px; height: 16px;}';
...
function show()
{
	...
	var src = img_obj.data("cke-saved-src");
...
function RefreshSize()
{
	...
	picContainer.style.width = (parseInt(contents.$.style.width, 10) -20) + "px";
...
{
	var target = e.data.$.target || e.data.$.srcElement;
	if ( target.nodeName == 'LEGEND' )
	{
		var fieldset = target.parentNode,
			toggledOff = !target.getAttribute( 'data-toggledOff' );
		target.setAttribute('data-toggledOff', (toggledOff ? '1' : ''));
		for( var i=0; i<fieldset.children.length; i++ )
		{
			var child = fieldset.children[ i ];
			if ( child != target )
				child.style.display = toggledOff ? 'none' : '';
		}
		fieldset.style.padding = toggledOff ? '1px' : '';
		RefreshSize();
	}
}
		...
		[
			{
				...
				[
						...
						[
							{
								...
								widths: ['20px','18px','18px','18px','26px','400px'],
								children:
								[
								{
										type:'html',
										...
										html: '<img src="' + spacerImg + '" style="background-position: 0 -64px;" class="imgmapButton">'
									},
									{
										type:'html',
										id:'btn_rect',
										onClick: function() { setMode('rect');},
										html: '<img src="' + spacerImg + '" style="background-position: 0 -128px;" class="imgmapButton">'
									},
									{
										type:'html',
										id:'btn_circle',
										onClick: function() { setMode('circle');},
										html: '<img src="' + spacerImg + '" style="background-position: 0 0;" class="imgmapButton">'
									},
									{
										type:'html',
										id:'btn_poly',
										onClick: function() { setMode('poly');},
										html: '<img src="' + spacerImg + '" style="background-position: 0 -96px;" class="imgmapButton">'
									},
									{
										type:'html',
										onClick: function() { myimgmap.removeArea(myimgmap.currentid); },
										html: '<img src="' + spacerImg + '" style="background-position: 0 -32px;" class="imgmapButton">'
									},
									{
										type:'html',
					...
					{
						type: areasContainer,
						label: lang.imagePreview,
						children :
						[
							{
								type:'html',
								html:'<div id="' + pic_containerId + '" style="overflow:auto;width:200px;height:300px;position:relative;"></div>'
							}
						]
					}
				]
